...
PRÓXIMO PASO: **(Luciano/Terminal)** `git add -A && git commit` + `clasp push -f` → en el editor `selfTest()` (confirmar **D4 verde** con los 3 asserts nuevos) → luego elegir el **fork del backlog**: (a) **cargar canal local de Vehemence** en la fuente (resuelve el caveat de datos / hallazgo #2) o (b) **conectar el resto de la cartera** (ya habilitado por el adaptador parametrizado #3). **PURGA DEL CONECTOR HECHA (19-jun):** 1 Crítico + 2 Altos + 3 menores → **6 parches aplicados y verificados offline** (node harness reproduce D4 verde + batch-delete + guard anti-wipe); detalle en Verificado/changelog. `19_conectores.js` + `09_selftest.js` **sin commitear** (sandbox no escribe `.git`). **ETAPA CONECTOR CERRADA (18-jun):** arco end-to-end probado sobre Vehemence — conectar (sin credenciales, mismo Workspace) → cargar (15 filas, validadas vs ERP) → dirigir (Director por objetivo) → **Analista + Vigía analizaron datos reales** (Analista: AOV $104.495, target $110-115k; Vigía: declive online −83% desde may-2025, colapso dic-2025). Sync en `corridaDiaria` (diario auto). **Caveat de datos:** `DB_VENTAS` ~100% online (canal local pendiente en la fuente) → el declive es de online, no del negocio total.
