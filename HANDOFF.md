...
**[27-ago · Code · Bloque A de SATO VIVIENTE]** Sesión aditiva: 0 cambios en `src/` y `voz/`, `/exec @57` intacto. Entregó F0 (auditoría de consumo: **GO, Haiku 4.5 = 2,1% del baseline semanal**), F1 offline (**el caching TC-10 hoy no cachea en ningún módulo** — bloques de 143 tok contra mínimos de 1.024/4.096), F2a (launchd del grafo corregido + `scripts/install-grafo-launchd.sh`), F2b (996 nodos, 3 warnings) y F2c (**el Cerebro es canvas 2D, no Three.js ⇒ F10 reformulado**). Detalle en `~/Documents/Claude/Handoffs/2026-08/2026-08-27 - satori-os-sato-viviente-bloque-a.md` e informes en `out/`. Bloques B-G **gateados por el 2º reload de `agent.py`** (`out/pendientes-post-E2.md`).
> **Stress test + purga posteriores (misma sesión) encontraron 2 P0 —** ver `PLAN-REMEDIACION-2026-08-27.md`: (1) `install-grafo-launchd.sh` puede tumbar el Cerebro sin rollback (reproducido en vivo); (2) 6 primitivas de `07_util.js` (`appendFila`, `leerTabla`, `ensureSheet`, `getMaestro`, `abrirCliente`, `conLock`) son client-callable, sin `_soloOwner_` y **fuera de `ENDPOINTS_UI`, así que `securityScan_` es estructuralmente ciego a ellas**.
> **[27-ago · Olas 0-3 de remediación, CERRADAS]** `arnés 919/0` · `_verificar_index` OK + **35 breakpoints `@media` congelados** · Cerebro `running`+200 con instalador blindado (`plutil -lint` + rollback + guard de dominio). **R6 dio el número que reformula F6: la latencia de la voz es GAS, no el modelo** — A (LLM) p50 **1,08 s** vs B (round-trip a GAS) p50 **6,75 s** de un turno de 12,82 s ⇒ el criterio «primer token ≤900 ms» mide lo que no importa. ⛔ **Retractados: el «P0 de superficie RPC» era FALSO** (`_harness.js` D31 ya tenía la partición derivada del código; `google.script.run` no serializa un `Sheet`) y **R14 no se ejecuta con motivo escrito en el código**. Cierre: `~/Documents/Claude/Handoffs/2026-08/2026-08-27 - PLAN-REMEDIACION-CIERRE.md`.
> **[27-ago · ENCARGO POST-OLAS, CERRADO salvo F-CRM-Mobile]** `arnés 942/0` · 36 breakpoints congelados · reload consolidado de `agent.py` verificado (14:43:53 > mtime 14:35:42, plugin `anthropic 1.6.4` registrado). Entró **F3 identidad editable en caliente** (`docs/SATO-IDENTIDAD.md` como fuente única; editarla cambia a Sato **sin reload**), F4 drift-checker, **F5 Consejo GO 8/10**, F6 motor Claude cableado con flag `SATO_VOZ_MOTOR=openai` apagado, F7 Sato Ubicuo completo (4 endpoints + dock, `sato_ubicuo_on=no`), F10-a Cerebro (DPR, freshness, layout determinista, deep links), F11/F12/F-Diseño-Patrones. 🔴 **CORRECCIÓN CLAVE:** la palanca «Sonnet 5 cacheado 5× más barato» es de **Sato-in-GAS**, no de la voz — medido contra la API, el prefijo de la voz son **8.125 tok** y Haiku 4.5 cachea igual, cuesta la mitad y es más rápido ⇒ **voz = Haiku, GAS = Sonnet 5**. **F-CRM-Mobile NO se ejecutó** (port de 65 KB sobre el UI diario; R12 ya la protege). Cierre: `~/Documents/Claude/Handoffs/2026-08/2026-08-27 - SATO-VIVIENTE-POST-OLAS-CIERRE.md`.
> **[27-ago · SATO WEB SEARCH B1-B6, CERRADO]** `arnés 961/0` · `test_web_satori` 43/43 · `test_log_uso` 9/9. Sato-VOZ tiene **búsqueda y lectura web con whitelist Satori**, presupuesto de $10/mes ($0,01 gastado) y telemetría propia. 🔴 **PC-4 del encargo anterior era un VERDE FALSO: la voz NUNCA corrió sobre Claude** — el plugin arma su cliente con `httpx` y el SDK `anthropic` 1.x usa `httpx2`, así que caía a gpt-4o-mini, y `_log_uso` escribía la env var en vez del modelo que contestó, así que la métrica tapaba el error. ⇒ **los hit-rates de 84,7-99,5% son de gpt-4o-mini; el número de Haiku NO está medido** (cabo #1: se resuelve con una conversación de voz). Además: el dock era invisible por stacking context (`#centro` fixed z-50) ⇒ escalera nueva 230/240 congelada por asserts; el cache cliff está a **~9 tools**, no a 5 — **son las tools las que hacen que Haiku cachee**; y el timeout de `cerebroGrafo` no era del tenant sino de la **cola serializada de Apps Script**. ⚠ `/exec` **@58 tiene sólo B1** — B3 y B5 están en `/dev`, sin promover (§7 exige tu eyeball). Cierre: `~/Documents/Claude/Handoffs/2026-08/2026-08-27 - SATO-WEB-SEARCH-CIERRE.md`.
