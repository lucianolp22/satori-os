...
PRÓXIMO PASO: **correr `bash _f11_fix_code.sh` (dry-run → `--go`)** = fix del hallazgo de Luciano 08-jul: el CM tenía `overflow:hidden` → **Actividad de agentes y Calendario estaban renderizados DEBAJO del viewport, inalcanzables** (misma causa por la que la card Agenda de @19 jamás se vio). Ahora el centro escrolea (topbar sticky, fondos fixed quedan quietos). Además: card **Tareas** pasa de estados a **contextos reales** (Hoy=vence-hoy · Clientes · Periódicas · En curso, vía `tareas_ctx` en datosHoy). Verificación: `selfTest()` (D8b) + `/dev` **escroleando hacia abajo**. Si aprueba → promover `/exec` @20 (junta v11 + F1 + F1.1). Cierre próximo: borrar `src/99_tmp_tipos.js`. Gates 08-jul: IG monitor debe ACUMULAR · Barcelona Activa.
...
### No verificado
- **F1.1 (scroll del centro + card Tareas por contextos)**: en working tree, NO corrida en GAS (`_f11_fix_code.sh` pendiente) → selfTest D8b + eyeball ESCROLEANDO en `/dev`. El delta acumulado (v11+F1+F1.1) entra a prod al promover @20 POST-eyeball.
...
**Must:**
- Correr `bash _tareas_f1_code.sh` (dry-run → `--go`) + `setup()` + `tmp_poblarTipos()` + `selfTest()` (D8) + eyeball del Tablero en `/dev` → promover `/exec` @20 (junta v11 + F1).
- Próximo cierre: borrar `src/99_tmp_tipos.js` (temporal cumplido, patrón 99_tmp).
