...
PRÓXIMO PASO: **correr `bash _tareas_f1_code.sh` en Terminal (dry-run → `--go`)** = Tareas-v2 F1 a GAS HEAD → en el editor: **`setup()`** (agrega las 4 columnas a Tareas, aditivo) → **`tmp_poblarTipos()`** (espera `{tocadas: 23}`) → **`selfTest()`** (TODO OK con D8) → **eyeball del Tablero en `/dev`** (checklist en el output del script: quick-add con sigilos, filtros por tipo, chip ↻, drag periódica→Hecha renace). Si aprueba → promover `/exec` **@20** (junta v11 + F1). Recordatorio del próximo cierre: borrar `src/99_tmp_tipos.js` (patrón 99_tmp). Después: gates 08-jul (IG monitor debe ACUMULAR · Barcelona Activa) + B7 negocio + F2-F4 de Tareas-v2 con gatillo.
...
### No verificado
- **Tareas-v2 F1**: NO corrida en GAS (`_tareas_f1_code.sh` pendiente) → setup reconciliando columnas + tmp_poblarTipos + selfTest D8 + eyeball del board. El delta acumulado (v11 ya en /dev + F1) entra a prod al promover `/exec` (@20) POST-eyeball.
