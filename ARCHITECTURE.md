...
| `19_conectores.js` | **capa de conectores** (integración con los sistemas del cliente). Patrón PULL-AND-SYNC por contrato: el conector lee la fuente y escribe agregados en `Datos_operativos`, que es lo que ya leen los agentes — los agentes NO cambian y no hay read-live. Cada conector nace apagado en Config (`conector_*`) | `sincronizarConectores()`, `sincronizarVehemence()`, `probar/encender/apagar` por cliente, `mapearOperacionesGenerico_()` |
| `20_killswitch.js` | **kill switch unificado** (riel Bastión #7). Un interruptor congela TODA la actividad autónoma (`corridaDiaria`, `drenarCola`, `chequeoLivianoDirector`, `clasificarBandeja`) **y** el canal de voz: en pausa el `doPost` rechaza todas las tools, lecturas incluidas. Modo PAUSA OPERATIVA, no apagado destructivo | `_sistemaPausado_()`, `pausar()`, `reanudar()`, `smokeKill()` |
| `21_backup.js` | **backup semanal de los DATOS** (B3): copia íntegra del MAESTRO + cada Sheet cliente a una carpeta Drive fechada, en el Drive del owner. Incluye **restore drill** — un backup que nunca se restauró no es un backup | `backupSemanal()`, `backupAhora()`, `backupListar()`, `drillRestore()`, `smokeBackup()`, `instalarTriggerBackup()` |
| `23_evals.js` | **golden-set + runner de evals** (T3 · MÓDULO M · M4). Cubre el hueco que `selfTest` no puede: el selfTest verifica que el sistema NO REVIENTE; esto verifica que sigue **decidiendo bien**. Un cambio en el parser de voz o en el normalizador de cifras pasa el selfTest en verde y rompe la captura en silencio | `correrEvals()`, `correrEvalsConApi()`, `EVALS_FAMILIAS_DET` |
| `24_soul.js` | **SOUL: identidad operativa**. Las invariantes de conducta que valen para TODA superficie —voz, clasificador, agentes, brief, y lo que se escriba mañana—. Se REFERENCIAN por id (`soulPrompt_(['S1','S2'])`), no se re-escriben en cada prompt: una invariante duplicada diverge | `soulPrompt_()`, `SOUL` |
| `25_hilo.js` | **Hilo de trabajo por cliente** (TC-W1/W2/W4): el estado vivo del trabajo en cuatro secciones — PLAN, REAL, y el número del conector que lo respalda. Lista-contrato `HILO_SECCIONES`; todo texto de tenant pasa por `limpiarHostilTexto_` | `hiloCliente(id)`, `repararHilo()`, `_numeroConectorCliente_()` |
| `29_vigilancia.js` | **vigilancia multi-superficie** (TC-11 · A5): de vigilar 1 superficie a N declaradas por cliente en Config. Doctrina D26c — **GRIS = sin datos y vacío JAMÁS es verde**; cada color viaja anclado a su dato y un dato viejo DEGRADA a gris con nota. `corridaDiaria` persiste el resumen en Config para que el brief lo lea sin abrir Sheets de cliente | `vigilanciaCliente(id)` (endpoint Ficha 360), `vigilanciaCorrida_()`, puros: `_vigJuzgar_()`, `_vigUmbrales_()`, `_vigLineasBrief_()` |
| `30_correo.js` | **correo → triaje a Bandeja** (T7 · TC-6). Ley: `docs/SPEC-correo-T7.md` con su dictamen Bastión de 9 cláusulas. El correo NO es un módulo de correo: es `fuente='correo'` en Bandeja, clasificado por el clasificador que ya existe. Scope único `gmail.readonly`, **cero escritura sobre Gmail** (por eso el dedupe vive en la hoja `Correo_visto`, que guarda solo el id). Nace apagado (`correo_on=false`) | `correoTriaje()`, puros: `_extractoCorreo_()` (la anonimización), `_correoDebeCorrer_()`, `_correoDecidirMensaje_()` |
| `31_admin.js` | **administración propia** (TC-7 · F4a): Sheet propio `Satori OS — ADMIN`, fuera del roster (Satori no es tenant de sí misma). El resumen agrupa por jurisdicción **y moneda**, sin total global — sumar EUR con ARS miente. El **calendario fiscal se entrega vacío**: ni una fecha ni un modelo como ciertos, todo marcado «verificar con gestor/AEAT». Gate real de F4a abierto esperando las facturas 2026 | `adminSetup()`, `altaFactura/altaGasto/altaCobro()`, `adminResumenMes()`, puros: `_adminResumir_()`, `_calendarioFiscalPlaceholders_()` |

> ✅ Índice COMPLETO al 04-ago-2026 (TC-8): las 9 filas que faltaban entraron con sus módulos. Verificado por barrido `ls src/*.js` contra la tabla — cero módulos sin fila.
