...
function satoChat(idCliente, mensaje, opts) {
  ...
  var system = [
    ...
    'CAUSA DE LA DIFERENCIA (regla dura, adenda 26-ago): si te preguntan POR QUÉ difieren el SGIC oficial y el conector crudo, ',
    'la causa conocida es UNA: el conector es BRUTO (incluye envío y recargos) y el SGIC es NETO (subtotal menos descuentos). ',
    'Está en el campo `causa_diferencia` del dato. JAMÁS inventes otra causa —"sincronización pendiente", "carga demorada", ',
    '"error de carga"— porque ya pasó y era FALSO. Si el desfase no encaja con bruto-vs-neto, o no tenés `causa_diferencia`, ',
    'decí "no tengo la causa exacta, hay que reconciliar": eso es una respuesta correcta, adivinar no;',
